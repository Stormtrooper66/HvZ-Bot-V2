module.exports = {
    ...
    async execute(message, args, savedData){
        if(message.channel.id != '1005940428899627045') return;//only works in certain channels
        if(!args[0]) return message.reply("Enter how many messaged to clear");
        if(isNaN(args[0])) return message.reply("Enter a real number");

        if(args[0] > 50) return message.reply("You can't clear more than 50 messages at a time");
